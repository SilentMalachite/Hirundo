...
public struct Author: Codable, Sendable {
    ...
    public init(name: String, email: String? = nil, limits: Limits = Limits()) throws {
        // 名前の検証（簡素化）
        self.name = try ConfigValidation.validateNonEmptyAndLength(name, maxLength: limits.maxAuthorNameLength, fieldName: "Author name")
        
        // メールアドレスの検証（簡素化）
        if let email = email {
            let trimmedEmail = try ConfigValidation.validateLength(email, maxLength: limits.maxEmailLength, fieldName: "Email")
