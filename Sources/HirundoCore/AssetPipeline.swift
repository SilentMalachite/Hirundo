...
///
/// Only `cssOptions.minify` / `jsOptions.minify` are reachable from `config.yaml` (through
/// `features.minify`). The rest is library surface that is **not ready to be exposed**, and the
/// `build:` keys that used to suggest otherwise were removed rather than wired up:
///
/// - Nothing rewrites `href`/`src` in generated HTML or `url(...)` in CSS, so a fingerprinted or
///   concatenated asset is one no page loads.
/// - `enableFingerprinting` hashes the *source* bytes (below) but writes the *processed* ones,
///   so the hash does not identify what it names, and the manifest stores a bare filename
///   rather than a path.
/// - `concatenationRules` matching disagrees between `AssetFileManager.findFiles` and
///   `isConcatenatedFile`: a `js/*.js` rule bundles *and* copies the sources, while a bare
///   `*.js` rule drops them from the output entirely. Order is `sorted()` on absolute paths and
///   subdirectories are not walked.
/// - `enableSourceMaps` and the `sourceMap` processing options are read nowhere; no source map
///   is generated by any code path.
