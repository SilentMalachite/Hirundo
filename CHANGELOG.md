...
- **BREAKING**: the `build.enableAssetFingerprinting`, `enableSourceMaps`, `concatenateJS` and `concatenateCSS` keys were removed. They decoded into `Build` and were read by nothing, and none of them is one wire away from working: nothing rewrites the `href`/`src` references in generated HTML, so a fingerprinted or concatenated asset is one no page loads; the fingerprint hashes the source bytes while writing the processed ones; the concatenation matcher disagrees with its own file finder, so a `js/*.js` rule emits the bundle *and* the sources while a bare `*.js` rule drops the sources entirely; and no source map is generated by any code path. `hirundo validate` reports these keys if a configuration still carries them, and `AssetPipeline` now documents each defect
