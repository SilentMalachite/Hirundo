...
- Correction to the confinement note below: what is confined is the **pages and the assets**. `sitemap.xml`, `rss.xml`, `search-index.json` and `asset-manifest.json` are still written straight through `write(to:)` without the guard, so an outward-pointing link left at one of those names is followed rather than removed; and `AssetPipeline.write` still creates a destination's parents with `withIntermediateDirectories: true` rather than the component-by-component `OutputPathGuard.createDirectories`, so an intermediate link whose target does not exist yet is not detected. Both predate this release and both are recorded rather than fixed here
