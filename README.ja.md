...
- **🎨 テンプレート**: 21種類のカスタムフィルターを備えたStencilベースのテンプレートエンジン
...
- **⚡ シンプル**: 設定項目はトップレベル7キーのみ。管理すべきプラグインランタイムはありません
...
> `--force` を付けない限り、削除対象を一覧表示するだけです。出力ディレクトリは
> `config.yaml` の `build.outputDirectory` から読み取られ、無ければ `_site` になります。
>
> `--force` は出力ディレクトリを**削除せず中身だけを空にします**。`hirundo build --clean` と
> 同じ挙動で、実装も共有しています。`_site` をビルド用ボリュームへのシンボリックリンクに
> している構成ではリンクが残り、リンク先の中身だけが消えます。`--cache` で対象になる
> `.hirundo-cache` はディレクトリごと消します（設定されたルートではなく、このツール自身の
> ディレクトリなので）。失敗したときは報告して終了コード1で終わります。
...
- **オプションの各ブロックは一部のキーだけを書けます。** `features` / `limits` / `assets` /
  ...
  変えるために残り8つを書き直す必要はありません。解釈されないキーは `hirundo validate` が
  報告します。
- **値はデコードされるだけでなく検証されます。** `site.url` はスキームとホストを持つURLで
  ...
  なお `site.*` の長さ上限（タイトル、説明、URL、著者名、メール、言語コード）は `limits`
  ブロックから**読まれます**（`maxTitleLength`、`maxDescriptionLength`、`maxUrlLength`、
  `maxAuthorNameLength`、`maxEmailLength`、`maxLanguageCodeLength`）。`limits` を省略した
  場合のデフォルトは上記のとおり（200、500、2000、100、254、35）です。
...
| `number` | 数値の書式化 |
| `default` | 空値のときの代替値 |
| `escape`（別名 `e`） | 値をHTML用にエスケープ |

Stencilに自動エスケープはないため、エスケープを要求する手段が `escape` です。
`hirundo init` が書き出すテンプレートは、`{{ content }}` を除くすべての補間に適用して
います。`{{ content }}` と `markdown` フィルターの出力には**適用しないでください**——
どちらも既にHTMLで、エスケープは冪等ではないので、ページのソースがそのまま表示されます。
このフィルターが入る前に作ったサイトのテンプレートは自動では変わりません。
...
アセットパイプラインが読むのは `static/` の中だけです。シンボリックリンクは、解決先が
`static/` の中に収まる場合にのみたどります。外を指すリンクは stderr に警告を出して飛ばすため、
`static/vendor -> ../node_modules/pkg/dist` は何も公開せず、`static/leak.txt -> /etc/passwd`
のようなリンクがリンク先の中身をビルド成果物へ持ち出すこともありません。公開したいファイルは
コピーするか `static/` の中に置いてください。`static/` の中で完結するリンクは、出力では通常の
ファイルとして実体が書き出されます。壊れたリンクは、黙って飛ばさずビルドエラーにします。

`fingerprint` を有効にすると、`static/` のアセットの大半は `style-9f2a1c04b7e3d5a1.css` の
ような内容ハッシュ付きの名前で出力され、生成されたHTMLの `href` / `src` / `srcset`、CSSの
`url(...)` と `@import`、`<style>` の本文と `style` 属性がその名前を指すように書き換えられます。
対応表は `_site/asset-manifest.json` に書き出されます。古い世代のハッシュ名のファイルは
ビルドのたびに削除されます。判断材料は現在の `static/` の中身と前回のマニフェストの両方なので、
`static/images/` ごと消したアセットも配信されなくなります。これにより `hirundo serve` を
回し続けても出力が膨れません。ただし削除されるのはハッシュ名のファイルだけです。除外対象の
アセットを `static/` から消した場合、その出力はクリーンビルドまで残ります ── ハッシュ名でない
パスまで消すと、たまたま同じパスに生成されたページを消してしまう恐れがあるためです。
なおこの掃除はフラグが
...
- **参照とマニフェストの照合は逐語的で、パーセントデコードはしません。** `/images/my%20photo.jpg`
  ...
  警告無しでそのまま通されます（下記の行き先の無いスタイルシート参照だけが警告を出す例外です）。
- **JavaScript内の参照は書き換えません。** `fetch("/images/logo.png")` のような文字列が参照
  かどうかは静的には判定できないためです。JSからアセットを参照する場合は
  `asset-manifest.json` を読んでください。
- **互いに参照しあうスタイルシートは元の名前のまま出力されます。** CSSからCSSへの参照
  （`@import url("other.css")` と、`url(...)` を伴わない `@import "other.css"` の両方）は
  通常どおり書き換えられます。スタイルシートは依存順に処理されるため、参照する側を処理する
  時点で参照先のハッシュ名は確定しています。互いに（あるいは自分自身を）参照しあうものだけは
  どう並べても確定できないので、その分だけフィンガープリントを諦めて元の名前で出力し、参照も
  そのまま残します。こうすることで参照は解決したままになります。どのスタイルシートにも
  行き着かない参照は警告を出します。
- **決まったURLで取得されるファイルは、自動的にフィンガープリントの対象外になります。**
  `robots.txt`、`sitemap.xml`、`favicon.ico`、`CNAME`、`_headers`、`_redirects`、
  `.htaccess`、`ads.txt`、`app-ads.txt`、`sw.js`、`service-worker.js` と、`.well-known/`
  配下のすべてが対象です。これらは書き換えられる参照を経由せずURLで直接取得されるため、
  改名すると既知の名前へのリクエストがすべて404になります。組み込みの一覧に設定は不要です。
...
上記の組み込み一覧 ── `robots.txt`、`sitemap.xml`、`favicon.ico`、`CNAME`、`_headers`、
`_redirects`、`.htaccess`、`ads.txt`、`app-ads.txt`、`sw.js`、`service-worker.js`、
`.well-known/**` ── は、`config.yaml` に `assets:` ブロックがあるかどうかに関わらず常に
適用されます。除外するファイルを追加するには、`assets.fingerprintExclude` にパターンを
列挙します。
...
パターンは、static ディレクトリからの相対パスに対して `/` 区切りで照合されます。この相対パスは
先頭に `/` を持たないため、パターン側にも先頭のスラッシュは付けません。書いてしまっても
先頭の `/` または `./` は照合前に取り除かれます（`/robots.txt` と `robots.txt` は同じ意味に
なります）。

- `/` を含まないパターンは、深さに関わらずファイル名に一致します（`ads.txt` は `ads.txt` にも
  `vendor/ads.txt` にも一致します）。組み込みがすべてこの形なのは意図的で、`.htaccess` は
  Apache が**各ディレクトリで**読み、Service Worker は配信されたパスがそのまま制御範囲
  （スコープ）になるためです
