...
- **互いに参照しあうスタイルシートは元の名前のまま出力されます。** CSSからCSSへの参照
  ...
  行き着かない参照は警告を出します。
- **決まったURLで取得されるファイルは改名しません。** 出力のルート直下の `robots.txt`、
  `sitemap.xml`、`favicon.ico`、`CNAME`、`_headers`、`_redirects`、`ads.txt`、`app-ads.txt`、
  `sw.js`、`service-worker.js` と、深さを問わず `.well-known/` 配下のすべてが対象です。
  これらは書き換えられる参照を経由せずURLで直接取得されるため、元の名前のまま出力され、
  マニフェストの値もキーと同じになります。同じ名前でもサブディレクトリにあるもの
  （`docs/robots.txt`、`js/sw.js`）は、通常のアセットと同じようにハッシュ名になります。
