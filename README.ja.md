...
- **フィンガープリントは `static/` 配下のすべてのファイルを改名します。** これには、決まった
  既知のファイル名で取得され、どのページからも参照されない次のようなファイルも含まれます。
  `robots.txt`、`favicon.ico`、`CNAME`、`_headers`、`_redirects`、`.well-known/` 配下の
  ファイルなど。これらへの参照はどのページにも存在しないため書き換えようがなく、フラグを
  有効にすると各ファイルはハッシュ名でのみ配信されるようになり、既知の名前へのリクエストは
  404になります。サイトがこれらに依存している場合は `features.fingerprint` を有効にしないで
  ください（ブラウザが暗黙に送る `/favicon.ico` へのリクエストが最も気づきやすい例です）。
