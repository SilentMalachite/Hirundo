...
`--host` はサーバーが実際にバインドするアドレスで、数値アドレスのみ受け付けます。ホスト名を
渡すとエラーになります。デフォルトの `localhost` はIPv4のループバックアドレスに解決される
ため、接続できるのはこのマシンだけです。他のマシンからの接続を許可するには
`--host 0.0.0.0` を明示してください。ただしその場合、ライブリロード用WebSocketには認証が
無いため、信頼できるネットワークでのみ使用してください。
